...
export const ProjectListPage = () => {
    const { projects, deleteProject, getProjects } = useProject();
    ...
    const filteredProjects = projects.filter(project =>
        `${project.nombre} ${project.estado} ${project.cliente} ${project.ubicacion}`.toLowerCase().includes(searchQuery.toLowerCase())
    );

    return (
        <div className="container mx-auto p-6">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-gray-700">Lista de Proyectos</h2>
            </div>
            <div className="flex flex-wrap gap-2 items-center justify-center md:justify-between mb-4 p-4 bg-white rounded-xl shadow-md border border-gray-300">
                ...
                <div>
                    <NavLink to="/dashboard/registerProject" className="flex items-center gap-x-2 rounded-md px-4 py-2 text-sm font-semibold shadow-sm text-white bg-blue-600 hover:bg-blue-700">
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {
                                                filteredProjects.map((project) => (
                                                    <tr key={project.id}>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{project.cliente}</td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{project.ubicacion}</td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                            <div
                                                                className={`py-1.5 px-2.5 ${project.estado.includes('en curso')
                                                                    ? 'bg-emerald-50'
                                                                    : project.estado.includes('detenido')
                                                                        ? 'bg-yellow-50'
                                                                        : 'bg-red-50'
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    <circle
                                                                        ...
                                                                        r="2.5"
                                                                        fill={
                                                                            project.estado.includes('en curso')
                                                                                ? '#059669'
                                                                                : project.estado.includes('detenido')
                                                                                    ? '#ca8a04'
                                                                                    : '#dc2626'
                                                                ...
                                                                <span
                                                                    className={`font-medium text-xs ${project.estado.includes('en curso')
                                                                        ? 'text-emerald-600'
                                                                        : project.estado.includes('detenido')
                                                                            ? 'text-yellow-600'
                                                                            : 'text-red-600'
                                                        ...
                                                        <td className="px-6 py-4 text-sm font-medium text-gray-500">
                                                            <div className='flex space-x-2'>
                                                                <NavLink to={`../detailProject/${project.id}`} className="text-blue-600 hover:text-blue-900">
                                                                    <EyeIcon className="w-5 h-5" />
                                                                </NavLink>
                                                                <NavLink to={`../editProject/${project.id}`} className="text-yellow-600 hover:text-yellow-900">
