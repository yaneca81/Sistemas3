...
export const ConsumeListPage = () => {
    const { consumes, getConsumes, deleteConsume } = useConsume();
    ...
    const filteredConsumes = consumes.filter(consumes =>
        `${consumes.cantidad} ${consumes.fecha}`.toLowerCase().includes(searchQuery.toLowerCase())
    );

    return (
        <div className="container mx-auto p-6">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-gray-700">Lista de Usos</h2>
            </div>
            <div className="flex flex-wrap gap-2 items-center justify-center md:justify-between mb-4 p-4 bg-white rounded-xl shadow-md border border-gray-300">
                ...
                <div>
                    <NavLink to="/dashboard/registerConsume" className="flex items-center gap-x-2 rounded-md px-4 py-2 text-sm font-semibold shadow-sm text-white bg-blue-600 hover:bg-blue-700">
                                ...
                                <div className="overflow-x-auto w-full rounded-xl border border-gray-300 shadow-md">
                                    <table className="w-full rounded-xl bg-white">
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {
                                                filteredConsumes.map((project) => (
                                                    <tr key={project.id}>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{project.cantidad}</td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{project.fecha}</td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{project.tarea.descripcion}</td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{project.insumo.nombre}</td>
                                                        <td className="px-6 py-4 text-sm font-medium text-gray-500">
                                                            <div className='flex space-x-2'>
                                                                <NavLink to={`../detailConsume/${project.id}`} className="text-blue-600 hover:text-blue-900">
                                                                    <EyeIcon className="w-5 h-5" />
                                                                </NavLink>
                                                                <NavLink to={`../editConsume/${project.id}`} className="text-yellow-600 hover:text-yellow-900">
